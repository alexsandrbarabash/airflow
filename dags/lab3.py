...
def validate_data_handler(**kwargs):
    data = kwargs['ti'].xcom_pull(key="data")
    if not data:
        return False
    ...
    for row in data:
        ...
        if row[1] is None:
            return False
    print("Всі тести пройдено без помилок.")


with DAG(
    'example_postgres_python_v11',
    default_args={
        'owner': 'airflow',
    ...
    validate_data = PythonOperator(
        ...
        python_callable=validate_data_handler,
        provide_context=True,
    )

    fetch_data >> validate_data >> display_data
    fetch_data >> validate_data >> filter_and_aggregate_data
